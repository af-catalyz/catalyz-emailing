<h2>Ajouter à la liste des destinataires, les contacts n'ayant pas ouvert ouvert vos campagnes précédentes</h2>
<?php echo form_tag('@campaign-target-add-provider?slug='.$campaign->getSlug().'&provider='.$providerName, array('method' => 'POST')) ?>
			...
			<?php $countPerGroup = $campaigns['countPerGroup']; ?>
		...
		<div class="tab-content">
			<div class="tab-pane active" id="fragment-1">
				<?php if (!empty($campaigns['active']) ) {
					foreach ($campaigns['active'] as $k => $aCampaign) {
						$nb = '';
						if (!empty($countPerGroup[$aCampaign->getId()])) {
							$nb = sprintf('<span class="hint"> (%d personne%s)</span>', $countPerGroup[$aCampaign->getId()], $countPerGroup[$aCampaign->getId()]>1?'s':'');
						}

						printf('<label class="checkbox"><input type="checkbox" name="campaigns[]" value="%s"/>%s%s</label>',  $aCampaign->getId(), $aCampaign->getName() ,$nb);
					}
				}
else{
