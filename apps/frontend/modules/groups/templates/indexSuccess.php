...
<table class="table table-striped table-condensed">
  ...
  <tbody>
    <?php
	    	...
	    	printf('<tr%s>', $contact_group->getIsArchived()?' class="archived_elements"':'');
	    	printf('<td>%s&nbsp;%s</td>', $contact_group->getName(), html_entity_decode($contact_group->getCommentPopup()));
	    	...
	    	if ($activeContactCount > 0) {
	    		printf('<a href="%s">%d</a>', url_for('contact/list?group=' . $contact_group->getId()), $activeContactCount);
	    	}else{
	    		echo '0';
	    	}
	    	echo '</td>';
	    	...
	    	if ($contactCount > 0) {
	    		printf('<a href="%s">%d</a>', url_for('contact/list?group=' . $contact_group->getId()), $contactCount);
	    	} else {
	    		echo '0';
	    	}
	    	echo '</td>';
