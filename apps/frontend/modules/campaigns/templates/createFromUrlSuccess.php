<?php printf('<div class="page-header"><h1>%s <small>(%s)</small></h1></div>', __('Nouvelle campagne'), __('depuis une url')) ?>
<?php $errors = $form->getErrorSchema()->getErrors(); ?>

<form class="form-horizontal" action="<?php echo url_for('@campaigns_create_from_url') ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
?>
	...
	<?php
printf('<div class="control-group %s">', !empty($errors['url'])?'error':'');
?>
		<label class="control-label" for="input01">Adresse de la campagne</label>
		<div class="controls">
			<?php
echo $form['url'];
echo $form['url']->renderError();
printf('<br/><span class="help-inline">Exemple: %s</span>', sfConfig::get('app_app_url'));
?>
		</div>
	</div>
