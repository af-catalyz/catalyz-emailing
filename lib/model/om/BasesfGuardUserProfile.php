...
abstract class BasesfGuardUserProfile extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Landing[] Collection to store aggregation of Landing objects.
	 */
	protected $collLandings;

	/**
	 * @var        Criteria The criteria used to select the current contents of collLandings.
	 */
	private $lastLandingCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collLandings = null;
			$this->lastLandingCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collLandings !== null) {
				foreach ($this->collLandings as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getLandings() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addLanding($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collLandings collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addLandings()
	 */
	public function clearLandings()
	{
		$this->collLandings = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collLandings collection (array).
	 *
	 * By default this just sets the collLandings collection to an empty array (like clearcollLandings());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initLandings()
	{
		$this->collLandings = array();
	}

	/**
	 * Gets an array of Landing objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUserProfile has previously been saved, it will retrieve
	 * related Landings from storage. If this sfGuardUserProfile is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getLandings($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserProfilePeer::DATABASE_NAME);
		}
		...
		if ($this->collLandings === null) {
			if ($this->isNew()) {
			   $this->collLandings = array();
				...
				$criteria->add(LandingPeer::CREATED_BY, $this->id);

				LandingPeer::addSelectColumns($criteria);
				$this->collLandings = LandingPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				LandingPeer::addSelectColumns($criteria);
				if (!isset($this->lastLandingCriteria) || !$this->lastLandingCriteria->equals($criteria)) {
					$this->collLandings = LandingPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastLandingCriteria = $criteria;
		return $this->collLandings;
	}

	/**
	 * Returns the number of related Landing objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Landing objects.
	 * @throws     PropelException
	 */
	public function countLandings(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserProfilePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collLandings === null) {
			if ($this->isNew()) {
				...
				$count = LandingPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Method called to associate a Landing object to this object
	 * through the Landing foreign key attribute.
	 *
	 * @param      Landing $l Landing
	 * @return     void
	 * @throws     PropelException
	 */
	public function addLanding(Landing $l)
	{
		if ($this->collLandings === null) {
			$this->initLandings();
		}
		if (!in_array($l, $this->collLandings, true)) { // only add it if the **same** object is not already associated
			array_push($this->collLandings, $l);
			$l->setsfGuardUserProfile($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collLandings) {
				foreach ((array) $this->collLandings as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
