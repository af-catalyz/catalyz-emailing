<?php


/**
 * Skeleton subclass for representing a row from the 'campaign_contact_bounce' table.
 *
 *
 *
 * This class was autogenerated by Propel 1.4.2 on:
 *
 * jeu. 03 mai 2012 11:26:44 CEST
 *
 * You should add additional methods to this class to meet the
 * application requirements.  This class will only be generated as
 * long as it does not already exist in the output directory.
 *
 * @package    lib.model
 */
class CampaignContactBounce extends BaseCampaignContactBounce {
	public function getBounceLabel()
	{
		switch($this->getBounceType()){
			case catalyzemailingHandlebouncesTask::BOUNCE_HARD:
				return '<span class="badge badge-error">HARD</span>';
				break;
			case catalyzemailingHandlebouncesTask::BOUNCE_SOFT:
				return '<span class="badge badge-warning">SOFT</span>';
				break;
			default:
				return FALSE;;
		} // switch
	}
} // CampaignContactBounce
