...
class CampaignTemplateImporter_TypeMapper_Wysiwyg extends CampaignTemplateImporter_TypeMapper_Default {
    ...
    {
        return sprintf('<?php if (!empty(%1$s["%2$s"])) { $renderer = new CatalyzEmailRenderer("%3$s", "%4$s", "%5$s"); echo $renderer->renderWysiwyg(%1$s["%2$s"], "%4$s"); } ?>', $this->getAccessorName(), $this->name, $this->getFontFace(), $this->getLinkColor(), $this->getCssStyles());
    }
