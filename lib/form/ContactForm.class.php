...
class ContactForm extends BaseContactForm
{
	public function configure()
	{
		...
		//then edit enabled
		$CustomFieldsI = /*(CustomFields)*/CustomFields::instance();
		$customFields =  CatalyzEmailing::getCustomFields();
		foreach ($customFields as $key => $details) {
			$this->widgetSchema[$key] =  $CustomFieldsI->getCustomFieldsWidget($key, $details['type']);
			$this->validatorSchema[$key] = new sfValidatorString(array('max_length' => 255, 'required' => false));
			$labels[$key] = ContactPeer::getFieldLabel($key);

		}
		//endregion

		$this->validatorSchema['email'] = new sfValidatorEmail(array('required' => true), array('required' => 'L\'adresse email est requise','invalid' => 'L\'adresse email n\'est pas valide'));
