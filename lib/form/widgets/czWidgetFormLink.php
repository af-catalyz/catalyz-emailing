        ...
        if ($value['value'] != '') {
            if (substr($value['value'], 0, 15) == '/uploads/assets') {
                ...
                if (LandingPageUtils::isModuleAvailable()) {
                    ...
                    if (count($choices) > 0) {
                        foreach($choices as $choiceUrl => $choiceName) {
							if (strpos($value['value'], $choiceUrl) === 0) {
                                $result .= sprintf('Lien vers la page  la page d\'atterrissage suivante : %s</span>',  $choiceName);
                                $found = true;
                                break;
                            }
                        }
