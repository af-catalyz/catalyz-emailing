...
class comparexCampaign01CampaignTemplateHandler extends CampaignTemplateHandler {
	public function createEditWidget()
	{
		$formClass = 'comparexCampaign01Form';
	...
	public function compute($parameters)
	{
		...
		return get_partial('comparex/campaign01', array('parameters' => $parameters));
	}

	public function getEditPartialName(){
		return 'comparex/campaign01_edit';
	}

	public function computeTextVersion($parameters){
		sfContext::getInstance()->getConfiguration()->loadHelpers('Partial');
		return get_partial('comparex/campaign01_text', array('parameters' => $parameters));
	}
