...
class cciFormationCampaign01CampaignTemplateHandler extends CampaignTemplateHandler {
	public function createEditWidget()
	{
		$formClass = 'cciFormationCampaign01Form';
	...
	public function compute($parameters)
	{
		...
		return get_partial('cciFormation/campaign01', array('parameters' => $parameters));
	}

	public function getEditPartialName(){
		return 'cciFormation/campaign01_edit';
	}

	public function computeTextVersion($parameters){
		sfContext::getInstance()->getConfiguration()->loadHelpers('Partial');
		return get_partial('cciFormation/campaign01_text', array('parameters' => $parameters));
	}
